...
How to Run
Download or clone the anudip folder.
Open anudip/index.html in any modern browser (Chrome, Edge, Firefox).
No installation, build step, or server needed — it runs entirely in the browser.
How It Works
Fill in each field. An error message appears as soon as a field is left empty or filled in incorrectly.
The Register button re-checks every field before allowing submission.
On a valid submission, the form is replaced by a summary card showing everything you entered.
Click Register another student to reset the form for a fresh entry.
